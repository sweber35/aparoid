...
import { generateSequenceQuery } from './util.mjs';
...
export const handler = async (event) => {
  ...
  try {
      ...
      //     return {
      //         statusCode: 400,
      //         body: JSON.stringify({ error: 'Missing actions array' }),
      //     };
      // }
    ...
    const shineGrabStubsQuery = `
        ...
        ${matchId ? `WHERE matchId = ${matchId}` : ''}
    `;
    //   const ledgeDashResults = await runAthenaQuery(ledgedashStubsQuery);

      // switch(category) {
      //   case 'Ledge Dashes':
      //       results = await runAthenaQuery(ledgedashStubsQuery);
      //       break;
      //   case 'Shine Grabs':
      //       results = await runAthenaQuery(shineGrabStubsQuery);
      //       break;
      // }
